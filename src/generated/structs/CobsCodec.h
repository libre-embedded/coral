...
#pragma once
#ifndef CORAL_STRUCTS_COBSCODEC_H
#define CORAL_STRUCTS_COBSCODEC_H
...
struct [[gnu::packed]] CobsCodec
{
    /* Constant attributes. */
    static constexpr struct_id_t id = 1;   /*!< CobsCodec's identifier. */
    static constexpr std::size_t size = 0; /*!< CobsCodec's size in bytes. */
    ...
    auto operator<=>(const CobsCodec &) const = default;
    ...
    inline std::size_t encode(Buffer *buffer) const
    {
        *buffer = *raw_ro();
        ...
        reinterpret_cast<CobsCodec *>(buffer)->endian<endianness>();
...
static_assert(sizeof(CobsCodec) == CobsCodec::size);
static_assert(ifgen_struct<CobsCodec>);

/* Stream interfaces. */
inline byte_istream &operator>>(byte_istream &stream, CobsCodec &instance)
{
    stream.read(instance.raw()->data(), CobsCodec::size);
    return stream;
}
inline byte_ostream &operator<<(byte_ostream &stream,
                                const CobsCodec &instance)
{
    stream.write(instance.raw_ro()->data(), CobsCodec::size);
    return stream;
}
