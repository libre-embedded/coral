...
template <byte_size T, std::endian endianness> inline void handle_endian_p(T *)
{
}
template <byte_size T, std::endian endianness> inline T handle_endian(T elem)
{
    return elem;
}
...
{
    handle_endian_p<uint32_t, endianness>(reinterpret_cast<uint32_t *>(elem));
}
template <std::floating_point T, std::endian endianness>
inline T handle_endian(T elem)
    requires(sizeof(T) == sizeof(uint32_t))
{
    return std::bit_cast<T>(
        handle_endian<uint32_t, endianness>(std::bit_cast<uint32_t>(elem)));
}
...
{
    handle_endian_p<uint64_t, endianness>(reinterpret_cast<uint64_t *>(elem));
}
template <std::floating_point T, std::endian endianness>
inline T handle_endian(T elem)
    requires(sizeof(T) == sizeof(uint64_t))
{
    return std::bit_cast<T>(
        handle_endian<uint64_t, endianness>(std::bit_cast<uint64_t>(elem)));
}
    ...
    requires(std::is_enum_v<T>)
{
    ...
    handle_endian_p<underlying, endianness>(
...
inline T handle_endian(T elem)
    requires(std::is_enum_v<T>)
{
    return static_cast<T>(handle_endian<std::underlying_type_t<T>, endianness>(
...
concept ifgen_struct = requires {
    std::is_same_v<decltype(T::id), struct_id_t>;
